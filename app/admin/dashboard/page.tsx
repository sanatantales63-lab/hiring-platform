...
export default function AdminDashboard() {
 ...
 const [examRequests, setExamRequests] = useState<any[]>([]);
  ...
  const uniqueCities = Array.from(new Set(students.map(s => s.city).filter(Boolean))).sort();
  const uniqueQuals = Array.from(new Set(students.map(s => s.highestQualification).filter(Boolean))).sort();
  const uniqueExps = Array.from(new Set(students.map(s => s.experience).filter(Boolean)));
  const uniqueNotices = Array.from(new Set(students.map(s => s.noticePeriod).filter(Boolean)));
  ...
  const filteredMainStudents = students.filter(s => {
    ...
    if (advQual && !s.highestQualification?.toLowerCase().includes(advQual.toLowerCase())) match = false;
    ...
    if (advAgeRange && s.dob) {
      ...
      if (advAgeRange === "under22" && age >= 22) match = false;
      else if (advAgeRange === "22-25" && (age < 22 || age > 25)) match = false;
      else if (advAgeRange === "26-30" && (age < 26 || age > 30)) match = false;
      else if (advAgeRange === "31-35" && (age < 31 || age > 35)) match = false;
      else if (advAgeRange === "above35" && age <= 35) match = false;
    }

    // Skill search (manual text)
    if (advSkillSearch.trim()) {
      const q = advSkillSearch.toLowerCase().trim();
      const hasSkill = s.skills?.some((sk: string) => sk.toLowerCase().includes(q));
    ...
    if (advStatus) {
       ...
       else if (advStatus === "exam_pending" && s.examAccess !== "pending") match = false;
       else if (advStatus === "completed" && s.examAccess !== "completed") match = false;
    }
           ...
           <div className="animate-in fade-in duration-300">
             ...
             <Card className="mb-10">
                 ...
                 <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    ...
                    {/* Status */}
                    <select value={advStatus} onChange={(e) => setAdvStatus(e.target.value)} className="bg-white border border-slate-200 rounded-xl py-2.5 px-3 font-medium text-sm focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/10 outline-none text-slate-700 shadow-sm transition-all">
                       <option value="">🟢 All Status</option>
                       <option value="none">✨ Available</option>
                       <option value="hired">💼 Hired</option>
                       <option value="exam_pending">⏳ Exam Pending</option>
                       <option value="completed">✅ Test Done</option>
                    </select>

                    {/* City */}
                    <select value={advCity} onChange={(e) => setAdvCity(e.target.value)} className="bg-white border border-slate-200 rounded-xl py-2.5 px-3 font-medium text-sm focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/10 outline-none text-slate-700 shadow-sm transition-all">
                       <option value="">🗺️ Any City</option>
                       {uniqueCities.map((city: any, i) => <option key={i} value={city}>{city}</option>)}
                    </select>

                    {/* Qualification */}
                    <select value={advQual} onChange={(e) => setAdvQual(e.target.value)} className="bg-white border border-slate-200 rounded-xl py-2.5 px-3 font-medium text-sm focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/10 outline-none text-slate-700 shadow-sm transition-all">
                       <option value="">🎓 Any Qualification</option>
                       {uniqueQuals.map((q: any, i) => <option key={i} value={q}>{q}</option>)}
                    </select>

                    {/* Experience */}
                    <select value={advExp} onChange={(e) => setAdvExp(e.target.value)} className="bg-white border border-slate-200 rounded-xl py-2.5 px-3 font-medium text-sm focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/10 outline-none text-slate-700 shadow-sm transition-all">
                       <option value="">💼 Any Experience</option>
                       {uniqueExps.map((e: any, i) => <option key={i} value={e}>{e}</option>)}
                    </select>

                    {/* Notice Period */}
                    <select value={advNotice} onChange={(e) => setAdvNotice(e.target.value)} className="bg-white border border-slate-200 rounded-xl py-2.5 px-3 font-medium text-sm focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/10 outline-none text-slate-700 shadow-sm transition-all">
                       <option value="">⏰ Any Notice Period</option>
                       {uniqueNotices.map((n: any, i) => <option key={i} value={n}>{n}</option>)}
                    </select>

                    {/* Gender */}
                    <select value={advGender} onChange={(e) => setAdvGender(e.target.value)} className="bg-white border border-slate-200 rounded-xl py-2.5 px-3 font-medium text-sm focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/10 outline-none text-slate-700 shadow-sm transition-all">
                    ...
                    {/* Work Mode */}
                    <select value={advWorkMode} onChange={(e) => setAdvWorkMode(e.target.value)} className="bg-white border border-slate-200 rounded-xl py-2.5 px-3 font-medium text-sm focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/10 outline-none text-slate-700 shadow-sm transition-all">
                    ...
                    <select value={advAgeRange} onChange={(e) => setAdvAgeRange(e.target.value)} className="bg-white border border-slate-200 rounded-xl py-2.5 px-3 font-medium text-sm focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/10 outline-none text-slate-700 shadow-sm transition-all">
                       ...
                       <option value="under22">Under 22 yrs</option>
                       <option value="22-25">22 – 25 yrs</option>
                       <option value="26-30">26 – 30 yrs</option>
                       <option value="31-35">31 – 35 yrs</option>
                       <option value="above35">Above 35 yrs</option>
                    </select>

                    {/* Min AI Score */}
                    <select value={advMinScore} onChange={(e) => setAdvMinScore(e.target.value ? Number(e.target.value) : "")} className="bg-white border border-slate-200 rounded-xl py-2.5 px-3 font-medium text-sm focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/10 outline-none text-slate-700 shadow-sm transition-all">
                     ...
                     <div className="col-span-2 md:col-span-1 relative">
                        ...
                        <input
                          type="text"
                          value={advTextSearch}
                          onChange={(e) => setAdvTextSearch(e.target.value)}
                          placeholder="Search by Name or Unique ID..."
                          className="w-full bg-white border border-slate-200 rounded-xl py-2.5 pl-9 pr-4 font-medium text-sm focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/10 outline-none text-slate-700 shadow-sm transition-all"
                        />
                     </div>
                     ...
                     <div className="col-span-2 md:col-span-2 relative">
                        ...
                        <input
                          type="text"
                          value={advSkillSearch}
                          onChange={(e) => setAdvSkillSearch(e.target.value)}
                          placeholder="Filter by skill keyword (e.g. GST, Tally, Excel)..."
                          className="w-full bg-white border border-slate-200 rounded-xl py-2.5 pl-9 pr-4 font-medium text-sm focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/10 outline-none text-slate-700 shadow-sm transition-all"
                        />
                     </div>
                    ...
                    <button
                      ...
                      className="col-span-1 bg-slate-100 hover:bg-rose-50 border border-slate-200 hover:border-rose-200 text-slate-500 hover:text-rose-600 rounded-xl py-2.5 px-3 font-bold text-sm transition-all flex items-center justify-center gap-2"
                    >
                      <X size={14} /> Reset All
                    </button>
                 ...
                 {(advTextSearch || advCity || advQual || advExp || advStatus || advNotice || advGender || advWorkMode || advTravel || advAgeRange || advMinScore !== "" || advSkillSearch || aiSkills.length > 0) && (
                   <div className="mt-4 flex flex-wrap items-center gap-2 pt-4 border-t border-slate-100">
                     ...
                     {advStatus && <span className="bg-[var(--accent)] text-[var(--primary)] border border-[var(--primary)]/20 px-2.5 py-1 rounded-lg text-xs font-bold flex items-center gap-1">Status: {advStatus} <button onClick={() => setAdvStatus("")} className="ml-1">✕</button></span>}
                     {advCity && <span className="bg-[var(--accent)] text-[var(--primary)] border border-[var(--primary)]/20 px-2.5 py-1 rounded-lg text-xs font-bold flex items-center gap-1">📍 {advCity} <button onClick={() => setAdvCity("")} className="ml-1">✕</button></span>}
                     {advQual && <span className="bg-indigo-50 text-indigo-700 border border-indigo-200 px-2.5 py-1 rounded-lg text-xs font-bold flex items-center gap-1">🎓 {advQual} <button onClick={() => setAdvQual("")} className="ml-1">✕</button></span>}
                     {advExp && <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 px-2.5 py-1 rounded-lg text-xs font-bold flex items-center gap-1">💼 {advExp} <button onClick={() => setAdvExp("")} className="ml-1">✕</button></span>}
                     ...
                     {aiSkills.map((sk, i) => <span key={i} className="bg-[var(--primary)] text-white px-2.5 py-1 rounded-lg text-xs font-bold flex items-center gap-1">AI: {sk} <button onClick={() => setAiSkills(aiSkills.filter((x, j) => j !== i))} className="ml-1">✕</button></span>)}
                   </div>
                 )}
